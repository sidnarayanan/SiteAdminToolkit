...
def os_perform(action, *args, **kwargs):
    while True:
        try:
...
def list_folder(name, opt):
    ...
    return os_perform(lambda name=name, the_filter=the_filter : 
                                [listing for listing in os.listdir(name)
                                 if the_filter(os.path.join(name, listing))])
...
def get_mtime(name):
    ...
    return os_perform(lambda name=name : os.stat(name).st_mtime)
...
def get_file_size(name):
    ...
    return os_perform(lambda name=name : os.stat(name).st_size)
